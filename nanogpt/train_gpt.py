...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # self.c_proj = FastKANLayer(config.n_embd, config.n_embd)
...
for step in range(max_steps):
    ...
    print(f"step {step+1:4d} | loss: {loss.item():.6f} | norm: {norm:.4f} | dt: {(time.time() - t0)*1000:.2f}ms | tokens/sec: {tokens_per_sec:.2f}")
